...
import org.hermeslauncher.app.vault.ShadePolicy
...
object HomeSearchRank {
    ...
    fun query(
        ...
        if (trimmed.isEmpty()) {
            val unread = inbox.filter {
                it.unread && !it.archived && ShadePolicy.hasSubject(it.title, it.conversationTitle)
            }
        ...
        val matchedInbox = inbox.filter { item ->
            !item.archived &&
                ...
                textOf(item).contains(trimmed, ignoreCase = true)
