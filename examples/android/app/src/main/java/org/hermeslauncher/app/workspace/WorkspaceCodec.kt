...
object WorkspaceCodec {
    ...
    private fun resolve(
        ...
        val withPods = if (screens.any { it.kind == WorkspaceKind.PODCASTS }) {
            screens
        } else {
            listOf(WorkspaceScreen(WorkspaceModel.PODCASTS_SCREEN_ID, WorkspaceKind.PODCASTS)) + screens
        }
        val ids = withPods.map { it.id }
        val inbox = withPods.firstOrNull { it.kind == WorkspaceKind.INBOX }?.id
        val homeId = when {
            home != null && home in ids -> home
            inbox != null -> inbox
            else -> ids.first()
        }
        return WorkspaceModel(screenIds = ids, homeScreenId = homeId, screens = withPods)
    }
}
