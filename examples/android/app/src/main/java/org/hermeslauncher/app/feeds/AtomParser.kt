...
object AtomParser {
    fun parse(feed: Element): List<FeedItem> {
        ...
        return XmlDocuments.elementsNamed(feed, "entry").mapIndexed { index, entry ->
            ...
            val html = XmlDocuments.firstChildText(entry, "content", "summary")
            FeedItem(
                ...
                imageUrl = ArticleImages.fromRss(
                    XmlDocuments.attr(entry, "media:thumbnail", "url")
                        ?: XmlDocuments.attr(entry, "media:content", "url"),
                    href(entry, "enclosure"),
                    mime(entry, "enclosure"),
                    html,
                ),
            )
        }
