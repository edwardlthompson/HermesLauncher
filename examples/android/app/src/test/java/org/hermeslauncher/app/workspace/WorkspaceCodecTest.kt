...
class WorkspaceModelTest {
    ...
    fun migrateTwoPagesKeepsWidgetIdsAndHomeInbox() {
        ...
        assertEquals(WorkspaceKind.PODCASTS, model.screens.first().kind)
        assertEquals(WorkspaceKind.FEEDS, model.screens[1].kind)
        assertEquals(WorkspaceKind.INBOX, model.screens[2].kind)
        assertEquals(
            listOf(
                WorkspaceModel.PODCASTS_SCREEN_ID,
        ...
        assertEquals(2, model.pagerIndex(WorkspaceModel.INBOX_SCREEN_ID))
        assertEquals(1, model.pagerIndex(WorkspaceModel.FEEDS_SCREEN_ID))
        assertEquals(0, model.pagerIndex(WorkspaceModel.PODCASTS_SCREEN_ID))
        ...
        assertEquals(1, model.widgetPageAt(3))
        assertEquals(0, model.widgetPageAt(2))
...
class WorkspaceCodecTest {
    ...
    fun missingHomeUsesFirstInbox() {
        ...
        assertEquals(WorkspaceModel.INBOX_SCREEN_ID, bundle.model.homeScreenId)
        assertEquals(WorkspaceKind.PODCASTS, bundle.model.screens.first().kind)
        assertEquals(2, bundle.model.pagerIndex(WorkspaceModel.INBOX_SCREEN_ID))
    }
