...
public class AlphabeticalAppsList<T extends Context & ActivityContext> implements
     ...
     */
    public static volatile int sSearchAppCap = Integer.MAX_VALUE;

    public boolean setSearchResults(ArrayList<AdapterItem> results) {
        if (results != null && sSearchAppCap > 0 && results.size() > sSearchAppCap) {
            results = new ArrayList<>(results.subList(0, sSearchAppCap));
        }
