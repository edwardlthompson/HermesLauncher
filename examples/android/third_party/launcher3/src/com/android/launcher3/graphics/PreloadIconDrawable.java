...
public class PreloadIconDrawable extends FastBitmapDrawable {
    ...
    public PreloadIconDrawable(
        ...
        Hct iconHct = Hct.fromInt(primaryIconColor);
        mProgressColor = Hct.from(
                iconHct.getHue(),
                iconHct.getChroma(),
                isDarkMode ? Math.max(iconHct.getTone(), 55) : Math.min(iconHct.getTone(), 40)
        ).toInt();
        mTrackColor = Hct.from(iconHct.getHue(), 16, isDarkMode ? 30 : 90).toInt();
        mPlateColor = Hct.from(
                iconHct.getHue(),
                isDarkMode ? 36 : 24,
