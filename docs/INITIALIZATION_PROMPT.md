...
- **In-app About (Module A):** `[AGENT]` scaffold `AboutScreen` with version, quiet **Donate via Venmo**, and Continuum-method launch prompts (`docs/features/donations-updates.md`). Persist `installed_artifact_format` (`apk`) on first run. Compare `{prefix}-X.Y.Z-foss.apk` filenames from GitHub assets, not git tags. Install opens the asset URL (fallback: release page). No Play Core or proprietary in-app update SDKs.
...
- **In-App About Screen & App Update Checker (product UI — not GitHub About, not template checker):**
  - **`[AGENT]`** Scaffold an accessible **About** screen reachable from primary navigation: app name, version, license/project URL, quiet **Donate via Venmo**.
  ...
  - **Donations:** default public Venmo link in `donations.json`; `[HUMAN]` may change it at init; external browser only; no donation tracking.
